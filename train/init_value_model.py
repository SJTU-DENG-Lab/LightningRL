"""
Value Model Initialization Script
...
def main():
    ...
    # Explicitly initialize value_head to zero
    with torch.no_grad():
        getattr(value_model, "value_head").weight.zero_()
    cprint("[Value Model Init] Value head initialized to zero", "green")

    # Save checkpoint
    cprint(f"[Value Model Init] Saving value model to: {config.model.optimized_value_name}", "green")

    # Save checkpoint first (this creates the directory)
