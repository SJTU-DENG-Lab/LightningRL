...
class TrainDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        step_rewards_all=None,    # (B,) list of dicts, per-step rewards for dense reward
        dense_reward_enabled=False,
...
def main():
    ...
    if kl_in_reward_enabled and current_epoch > 1:
        ...
        if os.path.exists(kl_file):
            kl_penalty_data = torch.load(kl_file)
            logger.info(f"Loaded pre-computed KL penalty from {kl_file}")
        else:
    ...
    # Check if dense reward is enabled
    ...
    step_map_list = []
    reward_list = []
    step_rewards_list = []  # List of dicts for dense reward
    ...
    for x in dataset_load:
        ...
        # Load step_rewards if available (for dense reward)
        if "step_rewards" in x and dense_reward_enabled:
    ...
    dataset_lm = TrainDataset(
        ...
        dense_reward_enabled=dense_reward_enabled,
    ...
    if kl_penalty_data is not None and kl_in_reward_enabled:
        ...
        # Map KL data to dataset rows using seq_ids
        # kl_penalty_data structure: {sample_idx: {response_idx: kl_tensor}}
        # dataset rows have seq_ids that reference original samples
        for row_idx in range(len(dataset_lm)):
            seq_id = int(dataset_lm.seq_ids[row_idx].item())
            # Determine which sample and response this row belongs to
            # seq_id directly corresponds to the index in the flattened (sample, response) list
            # We need to figure out sample_idx and response_idx
            # Since data is organized as: sample_0_resp_0, sample_0_resp_1, ..., sample_1_resp_0, ...
        ...
        logger.info("KL penalty filled from pre-computed data")
    ...
    #             Inference         #
    #################################
    logger.info(f"***** Running inference (batch_size={batch_size_value}) *****")
    ...
    def compute_returns_and_advantages_from_fragments(
        ...
        lam: float,
        *,
        dynamic_lambda_enabled: bool = False,
        dynamic_lambda_alpha: float = 0.05,
        ...
        Supports both sparse reward (only last step) and dense reward (all steps).
        Supports dynamic lambda: λ = 1 - 1/(α·L) where L is response length.
        ...
        for s in range(B):
            ...
            # Dense reward: assign reward to each step based on step_rewards
            if dataset.dense_reward_enabled and dataset.step_rewards_all is not None and len(dataset.step_rewards_all) > s:
            ...
            if kl_in_reward_enabled and dataset.kl_penalty is not None:
                ...
                for row in rows_by_seq[s]:
                    ...
                    kl_filled[tail_mask] = True
                
                # Apply KL penalty to reward (only at trainable positions)
                r_resp = r_resp - kl_in_reward_beta * kl_resp * trainable_mask.float()
            ...
            # Compute dynamic lambda if enabled: λ = 1 - 1/(α·L)
            L_s = int(trainable_mask.sum().item())  # response length for this sequence
            if dynamic_lambda_enabled and dynamic_lambda_alpha * L_s > 1:
                lam_s = 1.0 - 1.0 / (dynamic_lambda_alpha * L_s)
                lam_s = max(0.0, min(1.0, lam_s))  # clamp to [0, 1]
            else:
                lam_s = lam  # use fixed lambda

            A_star = torch.zeros(S, dtype=torch.float32)
            for i in range(S-1, -1, -1):
                A_star[i] = delta_star[i] + (gamma * lam_s * A_star[i+1] if i+1 < S else 0.0)
            ...
            for pos in torch.nonzero(trainable_mask, as_tuple=False).flatten().tolist():
                ...
                A_resp[pos] = (rj - V_resp[pos]) + gamma * V_next + gamma * lam_s * A_next
            ...
            # Assertion 1: when gamma=lambda=1 (and dynamic_lambda is disabled),
            # For sparse reward: R_j equals the sequence reward (all tokens same)
            # For dense reward: R_j for step i = sum of step_rewards from step i to last step
            # In both cases: A_j = R_j - V_j^{old}
            # Skip this assertion when dynamic_lambda is enabled (lambda is no longer fixed)
            if abs(gamma - 1.0) < 1e-8 and abs(lam - 1.0) < 1e-8 and not dynamic_lambda_enabled:
                ...
                if dataset.dense_reward_enabled and dataset.step_rewards_all is not None and len(dataset.step_rewards_all) > s:
                    step_rewards_s = dataset.step_rewards_all[s]
                    if step_rewards_s:
                        # For dense reward: R for step i = sum of rewards from step i to last step
                        ...
                        # Fall back to sparse: all tokens get the same reward
                        expected_R_resp[trainable_mask] = dataset.per_seq_reward[s].item()
                else:
                    # Sparse reward: all tokens get the same reward
                    expected_R_resp[trainable_mask] = dataset.per_seq_reward[s].item()
            ...
            # Assertion 2: when gamma=1, lambda=0 (and dynamic_lambda is disabled),
            # A_j = r_j - V_j^{old} + V_{t_j+1}^{*,old}
            # Skip this assertion when dynamic_lambda is enabled (lambda is no longer fixed)
            if abs(gamma - 1.0) < 1e-8 and abs(lam - 0.0) < 1e-8 and not dynamic_lambda_enabled:
    ...
    dynamic_lambda_enabled = OmegaConf.select(config, "dynamic_lambda.enabled", default=False)
    dynamic_lambda_alpha = OmegaConf.select(config, "dynamic_lambda.alpha", default=0.05)
    kl_in_reward_beta = OmegaConf.select(config, "kl_in_reward.beta", default=0.01)
    compute_returns_and_advantages_from_fragments(
        dataset_lm, gam, lam, 
        dynamic_lambda_enabled=dynamic_lambda_enabled,
        dynamic_lambda_alpha=dynamic_lambda_alpha,
    ...
    # Value clipping epsilon (PPO-style)
    value_clip_eps = OmegaConf.select(config, "training.value_clip_eps", default=0.2)
    ...
    def forward_process(extended_input_ids, p_mask, tok_idx_ext, Return, old_values):
        ...
        # PPO-style value clipping: prevent large value updates
        # V_clip = V_old + clip(V_new - V_old, -eps, eps)
        values_clipped = old_values + (values - old_values).clamp(-value_clip_eps, value_clip_eps)
        
        # Unclipped loss
        loss_unclipped = ((values - Return) ** 2) * p_mask
        # Clipped loss
        loss_clipped = ((values_clipped - Return) ** 2) * p_mask
        ...
        loss = loss.sum() / total_valid_tokens
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            del loss_lm
            del extended_input_ids, p_mask, tok_idx_ext, Return, old_values
            torch.cuda.empty_cache()
    ...
    if metrics_fp is not None:
        metrics_fp.close()
        logger.info(f"Closed metrics file: {metrics_file}")
