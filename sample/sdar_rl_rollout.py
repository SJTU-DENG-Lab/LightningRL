...
if __name__ == "__main__":
    ...
    # === DiRL prompt format support ===
    use_system_prompt = getattr(config.rollout, 'use_system_prompt', False)
    cprint(f"Prompt format: {'DiRL (system+user+assistant)' if use_system_prompt else 'Original (user+assistant)'}", "cyan")
    ...
    if OmegaConf.select(config, "rollout.stop_token_list", default=MISSING) is not MISSING:
        ...
        cprint(f"Using default stop_token_list for DiRL: {default_stop_tokens} -> token_ids: {stop_token_ids}", "cyan")
