...
def _process_temperature_(scores: torch.Tensor, temperature: torch.Tensor):
    ...
    Includes defensive shape checking to handle edge cases where
    scores and temperature batch sizes don't match (e.g., in DLLM).
    """
    temperature = temperature.to(scores.dtype)
    # Defensive check: ensure temperature matches scores batch size
    if temperature.size(0) != scores.size(0):
        temperature = temperature[:scores.size(0)]
...
def _filter_topp_sorted_(scores: torch.Tensor, topp: torch.Tensor, filter_value: float = -float('inf')):
    ...
    # Defensive check: ensure batch sizes match
    batch_size = scores.size(0)
    if topp.size(0) != batch_size:
        topp = topp[:batch_size]
...
def _filter_minp_sorted_(scores: torch.Tensor, minp: torch.Tensor, filter_value: float = -float('inf')):
    ...
    # Defensive check: ensure batch sizes match
    batch_size = scores.size(0)
    if minp.size(0) != batch_size:
        minp = minp[:batch_size]
    softmax_scores = scores.softmax(-1)
