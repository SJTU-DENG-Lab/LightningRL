...
class DLLMStoppingCriteria(StoppingCriteria):
    ...
    def step(self,
        ...
        # Initialize to not stop; only check whether to stop when block is fully unmasked
        ...
        # Only check the length stopping condition when the block is fully unmasked
...
class DLLMModelAgentStrategy(ModelAgentStrategy):
    ...
    def slice_outputs(self, inputs: torch.Tensor, seq_length: torch.LongTensor) -> torch.Tensor:
        ...
        Ensures the output always has exactly batch_size * block_length rows
        to match the expanded sampling_inputs tensors.
        """
        block_length = self.block_size
        # batch size = 1
        if len(seq_length) == 1:
            result = inputs[-block_length:]
            # Fix: ensure result has exactly block_length rows
            # This handles edge cases where inputs has fewer tokens than block_length
            actual_rows = result.size(0)
            if actual_rows < block_length:
                # Pad with the last row repeated to reach block_length
                padding_needed = block_length - actual_rows
                padding = result[-1:].expand(padding_needed, -1)
