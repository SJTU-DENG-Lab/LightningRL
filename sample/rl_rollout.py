...
def _kl_worker(
    gpu_id, data_chunk, policy_model_path, ref_model_path, tokenizer_path, batch_size, max_length, result_queue
):
    """
    ...
    try:
        ...
        # Load policy model
        policy_model = SDARForCausalLM.from_pretrained(policy_model_path, trust_remote_code=True, torch_dtype="auto")
        ...
        for batch_start in tqdm(
            range(0, len(all_items), batch_size), desc=f"GPU {gpu_id}", disable=(gpu_id != 0), total=num_batches
        ):
        ...
        if sequential_sampling:
            ...
            if completed_pass:
                ...
                if is_pretraining_file.exists():
                    cprint("[Sequential Sampling] Completed full dataset pass! (Pretraining: NO reshuffling)", "cyan")
                    # Reset cursor to 0 for next pass (keep same order)
                    cursor_file = Path("../" + project_name) / "temp_data" / "data_cursor.txt"
                    cursor_file.write_text("0")
                    cprint("[Sequential Sampling] Reset cursor to 0 (pretraining mode, keeping order)", "cyan")
                else:
                    cprint("[Sequential Sampling] Completed full dataset pass! Reshuffling...", "cyan")
