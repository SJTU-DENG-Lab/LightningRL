...
class OutputFilter:
    ...
    def write(self, text):
        # Filter rules:
        # 1. Warnings starting with 🚨 (transformers docstring)
        # 2. Config not found messages
        # 3. Triton autotune logs (BLOCKS_ARE_CONTIGUOUS, triton_flex_attention, AUTOTUNE benchmarking)
        filtered_keywords = [
            "🚨",
    ...
    # Proxy all attributes needed by tqdm
    def isatty(self):
...
class SDARForCausalLM(SDARPreTrainedModel, GenerationMixin):
    ...
    def prepare_for_bd_training(self, inputs_ids, position_ids, prompt_mask, masked_indices=None, p_mask_input=None):
        """
        Prepare block diffusion training data.
        ...
        # Compute p_to_keep - maps p_mask from original space to extracted space
        p_to_keep = None
        ...
        # Built based on the number of segments per sample
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.training:
            ...
            if compute_rl_loss:
                ...
                # (removed if condition so variable is also available during KL loss computation)
                ...
                # In prepare_for_bd_training, extracted positions correspond to True positions in logits_to_keep_half
                p_to_keep_real = logits_to_keep_half[p_mask_real]  # (M,) bool

                # Select logits using p_to_keep (in the correct extracted space)
                ...
                # Compute entropy if requested (no_grad to avoid interfering with gradient checkpointing)
                entropy_value = None
                ...
                # Map to extracted space
                adv_p = adv_expanded[p_mask_real][logits_to_keep_half[p_mask_real]]

                # old logp - needs to be mapped to extracted space
                ...
                if rl_correctness is not None and rl_correctness.any():
                    ...
                    if correct_p.any():
                        logp_correct = logp_p[correct_p]  # Extract logp for correct samples

                        # ===== Select reduction method based on nll_reduction_mode =====
                        if nll_reduction_mode == "sequence":
                            # Sequence-level: first average within each sequence, then average across all sequences
                            # Re-extract batch_indices (after applying correct_p filter)
                            batch_indices_correct = batch_indices_p[correct_p]
                            ...
                            seq_nll_sum.scatter_add_(0, batch_indices_correct, -logp_correct)

                            # Normalize: divide by number of correct tokens per sequence (avoid div by zero)
                            seq_nll_count_safe = torch.clamp(seq_nll_count, min=1)
                            ...
                            nll_loss = rl_nll_weight * (-logp_correct.mean())
                ...
                if rl_kl_beta > 0 and rl_logp_ref_tok is not None:
                    ...
                    if logp_p_valid.numel() == 0:
                        kl_loss = torch.tensor(0.0, device=device)
                    else:
                        # Compute KL only on valid positions
                        kl_seq_p = logp_p_valid - logp_ref_p

                        # Apply KL estimator transformation
                        # K1: standard KL (no transformation)
                        # K2: quadratic estimator (ESPO: best for sequence-level, no exp, stable)
                        ...
                        # k1: kl_seq_p remains unchanged (logp_new - logp_ref)

                        # ===== Select KL reduction method based on kl_reduction_mode =====
                        if kl_reduction_mode == "sequence":
                            # Sequence-level: first average within each sequence, then average across all sequences

                            # Re-extract batch_indices (after applying valid_token_mask)
                            batch_indices_valid = batch_indices_p[valid_token_mask]
                            ...
                            seq_kl_sum.scatter_add_(0, batch_indices_valid, kl_seq_p)

                            # Normalize: divide by number of valid tokens per sequence (avoid div by zero)
                            seq_kl_count_safe = torch.clamp(seq_kl_count, min=1)
                ...
                # PPO loss with asymmetric clipping
                ...
                if policy_reduction_mode == "sequence":
