...
def evaluate_function_dataset(data: list[dict], n_workers: int | None = None):
    ...
    with (
        ...
        tqdm(
            total=len(tasks) * len(data[0]["test_list"]),
            desc=f"Function tests ({n_workers} threads)",
            dynamic_ncols=True,
            mininterval=0.1,
            miniters=1,
        ) as pbar,
...
def evaluate_stdio_dataset(data: list[dict], num_chunks: int):
    ...
    exe_results = run_scripts_with_chunk(code_list, inp_list, tl_list, worker_stdio, num_chunks)
...
def main():
    ...
    output = {"metadata": raw.get("metadata", {}), "data": data} if isinstance(raw, dict) and "data" in raw else data
